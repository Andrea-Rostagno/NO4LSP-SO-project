...
for j=n_NewtonModified
    ...
    if fd == 1
            ...
            for increment = h
                ...
                tic;
                [~, f_min, iter_bar, min_hist_bar] = modified_newton(extended_rosenbrock,grad_f,hess_f,x_bar,tol,max_iter,name,fd,increment,0);
                t = toc;
                fprintf('f_min = %.6f\n | iter = %d | tempo = %.2fs\n', f_min, iter_bar, t);
                ...
                for i = 1:num_points
                    ...
                    fprintf('\n--- Test %d (x0 #%d) ---\n', i, i);
                    tic;
                    [~, f_min, iter, min_hist] = modified_newton(extended_rosenbrock,grad_f,hess_f,x0,tol,max_iter,name,fd,increment,0);
                    t = toc;
                    fprintf('f_min = %.6f\n | iter = %d | tempo = %.2fs\n', f_min, iter, t);
                ...
                fprintf('\n--- TEST SU VALORE X_BAR ---\n');
                tic;
                [x_min, f_min, iter_bar_abs, min_hist_bar_abs] = modified_newton(extended_rosenbrock,grad_f,hess_f,x_bar,tol,max_iter,name,fd,increment,1);
                t = toc;
                ...
                for i = 1:num_points
                    ...
                    fprintf('\n--- Test %d (x0 #%d) ---\n', i, i);
                    tic;
                    [x_min, f_min, iter, min_hist] = modified_newton(extended_rosenbrock,grad_f,hess_f,x0,tol,max_iter,name,fd,increment,1);
                    t = toc;
                    fprintf('f_min = %.6f\n | iter = %d | tempo = %.2fs\n', f_min, iter, t);
                    rho = compute_ecr_from_history(min_hist, 0); % se f* = 0
                    fprintf('rho ≈ %.4f\n\n', rho);
    ...
    if fd == 0
        ...
        fprintf('\n--- TEST SU VALORE X_BAR ---\n');
        tic;
        [x_min, f_min, iter_bar, min_hist_bar] = modified_newton(extended_rosenbrock,grad_f,hess_f,x_bar,tol,max_iter,name,fd,[],[]);
        t = toc;
        fprintf('f_min = %.6f\n | iter = %d | tempo = %.2fs\n', f_min, iter_bar, t);
        ...
        for i = 1:num_points
            ...
            fprintf('\n--- Test %d (x0 #%d) ---\n', i, i);
            tic;
            [x_min, f_min, iter, min_hist] = modified_newton(extended_rosenbrock,grad_f,hess_f,x0,tol,max_iter,name,fd,[],[]);
            t = toc;
            fprintf('f_min = %.6f\n | iter = %d | tempo = %.2fs\n', f_min, iter, t);
